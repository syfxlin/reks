...
export type ModifyFields = Omit<Fields, "date" | "datetime"> & {
  date: (
    config: Parameters<Fields["date"]>[0] & { updatingValue?: string | { kind: "now" } },
  ) => ReturnType<Fields["date"]>;
  datetime: (
    config: Parameters<Fields["datetime"]>[0] & { updatingValue?: string | { kind: "now" } },
